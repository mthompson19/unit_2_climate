...
plot(mass_Gt ~ decimal_date, data=ant_ice_loss_with_NA, ylab="Ice Mass Loss (Gt)", xlab="Year", type='l', ylim = range(grn_ice_loss_with_NA$mass_Gt, na.rm = T)) +
lines((mass_Gt+2*sigma_Gt) ~ decimal_date, data=ant_ice_loss_with_NA, type='l', lty='dashed') +
lines((mass_Gt-2*sigma_Gt) ~ decimal_date, data=ant_ice_loss_with_NA, type='l', lty='dashed') +
# add Greenland data to plot 
...
barplot(height=c(min(ant_ice_loss$mass_Gt)*(-1), min(grn_ice_loss$mass_Gt)*(-1)), names.arg=c("Antarctica","Greenland"), ylim=c(0,5000), ylab="Ice loss (Gt)")
```
...
```{r}
DiffAnt = c(max(ant_ice_loss$mass_Gt) - min(ant_ice_loss$mass_Gt))
AntTot = DiffAnt / max(ant_ice_loss$decimal_date - min(ant_ice_loss$decimal_date)) 

DiffGrn = c(max(grn_ice_loss$mass_Gt) - min(grn_ice_loss$mass_Gt))
GrnTot = DiffGrn / max(grn_ice_loss$decimal_date - min(grn_ice_loss$decimal_date)) 

pdf("figures/Ice_Melt_Per_Year.pdf", height = 5)
barplot(height = c(AntTot, GrnTot), names.arg=c("Antarctica","Greenland"), ylim = c(0,300), ylab="Ice loss per year (Gt/year)")
dev.off()
